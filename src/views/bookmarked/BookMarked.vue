  ...
  <div class="container">
    ...
    <right-side>
      ...
      <h6 class="title" v-if="this.requests">
        {{
          this.requests.length !== 0
            ? "Danh sách yêu cầu đã đánh dấu"
            : "Danh sách yêu cầu đã đánh dấu trống"
        }}
      </h6>
      <request-item
      ...
      <paginate
        ...
        :page-range="3"
        :click-handler="getRequestFollowings"
        :prev-text="'Trước'"
        :next-text="'Sau'"
...
// import BookmarkRequest from "@/views/bookmarked/BookmarkRequest.vue";
...
export default class BookMarked extends Vue {
  ...
  async mounted() {
    document.title = "Yêu cầu đã lưu";
    this.getUserFinanceOverview();
    this.getRequestFollowings();
  }
  ...
  async getRequestFollowings(pageIndex = 1) {
    const loader = this.$loading.show();
    try {
      await this.RequestInstance.fetchRequestFollowings([
        this.perPage,
        pageIndex
      ]);
      this.requests = this.RequestInstance.getBookmarkRequests.requests;
      if (this.requests !== null) {
        this.pageCount = Math.ceil(
          (this.RequestInstance.getBookmarkRequests.totalRecord ||
