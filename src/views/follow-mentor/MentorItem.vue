<template>
  <div
    ...
    @click="gotoProfile"
    v-if="mentorFollowing"
  >
    <div class="price">
      {{
        mentorFollowing.price
      ...
      <div class="bookmark" @click="toggleFollowRequest(mentorFollowing.id)">
        <font-awesome-icon
          class="text-warning"
          :icon="[mentorFollowing.followed ? 'fas' : 'far', 'bookmark']"
        />
      </div>
    </div>
    ...
    <div class="media">
      <ul>
        <li>
          <router-link :to="{ name: 'ProfileInfo' }" class="circle">
            <img
              class="avatar"
              v-if="mentorFollowing && mentorFollowing.avatar"
              :src="mentorFollowing.avatar"
              alt="avatar"
              width="50"
            />
            <img
              v-else-if="mentorFollowing && mentorFollowing.gender"
              class="avatar"
            ...
            <img
              ...
              width="50"
            />
            <p class="ribbon" v-if="mentorFollowing.anestMentor">Anest</p>
          </router-link>
        </li>
        <li>
          <rating-star
            :rating="mentorFollowing.rating"
            class="text-warning"
          ></rating-star>
        </li>
      </ul>
      ...
      <div class="media-body">
        <a href="#" class="mt-0 request-title"
          >{{ mentorFollowing.fullName }}
        </a>

        <p class="username">@{{ mentorFollowing.username }}</p>
        <p class="mentor-job">{{ mentorFollowing.job }}</p>
        <list-skill
          class="small"
          :class="displayType === 'small-card' ? 'x-small' : ''"
          :list-skill="mentorFollowing ? mentorFollowing.listSkill : []"
        />

        <slot></slot>
        <div class="request-content" v-if="displayType === 'detail'"></div>
      </div>
    </div>
    <confirm-modal
      :title="confirmTitle"
...
import { MentorFollowing } from "@/types/mentor";
...
export default class MentorItem extends Vue {
  @Prop() mentorFollowing: MentorFollowing | undefined;

  mentorAfterDelete: MentorFollowing | undefined;
  ...
  async toggleFollowRequest(mentorId: string) {
    const loader = this.$loading.show();
    try {
      const response = await mentorService.followOrUnFollowMentor(mentorId);
      if (response.status == 200) {
        if (this.mentorFollowing) {
          this.mentorFollowing.followed = !this.mentorFollowing?.followed;
          this.$emit(
            this.mentorFollowing.followed ? "bookmark" : "unbookmark",
            this.mentorFollowing
          );
        }
      }
    } catch (e) {
  ...
  gotoProfile() {
    if (!this.mentorFollowing) return;
    this.$router.push({
      name: "MentorProfile",
      params: { id: "" + this.mentorFollowing.id }
    });
  }
}
</script>
...
.bookmarked-request-content .media .media-body .row .col-8 {
