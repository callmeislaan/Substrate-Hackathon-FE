  ...
  <div class="container">
    ...
    <right-side>
      ...
      <h6 class="title" v-if="this.requests">
        {{
          this.requests.length !== 0
            ? "Danh sách yêu cầu được mời"
            : "Danh sách yêu cầu được mời trống"
        }}
      </h6>
      <request-item
        v-for="(item, index) in this.requests"
        :key="index"
        :displayType="'invitation-requests'"
        :request="item"
      >
      </request-item>
      <!-- Views -->
      ...
      <paginate
        ...
        :page-range="3"
        :click-handler="getInvitationRequests"
        :prev-text="'Trước'"
        :next-text="'Sau'"
...
export default class InvitationRequests extends Vue {
  ...
  async mounted() {
    document.title = "Yêu cầu được mời";
  ...
  async getInvitationRequests(pageIndex = 1) {
    const loader = this.$loading.show();
    try {
      await this.RequestInstance.getInvitationRequests([
        this.perPage,
        pageIndex
      ]);
      this.requests = this.RequestInstance.getRequestInvitation.requests;
      if (this.requests !== null) {
        this.pageCount = Math.ceil(
          (this.RequestInstance.getRequestInvitation.totalRecord ||
