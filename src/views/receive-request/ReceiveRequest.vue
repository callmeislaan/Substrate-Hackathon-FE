  ...
  <div class="container">
    ...
    <right-side>
      ...
      <h6 class="title" v-if="this.requests">
        {{
          this.requests.length !== 0
            ...
            : "Danh sách yêu cầu đã nhận trống"
        }}
      </h6>
      <request-item
      ...
      <paginate
        ...
        :page-range="3"
        :click-handler="getReceiveRequest"
        :prev-text="'Trước'"
        :next-text="'Sau'"
...
export default class ReceiveRequest extends Vue {
  ...
  async mounted() {
    document.title = "Yêu cầu đã nhận";
  ...
  async getReceiveRequest(pageIndex = 1) {
    const loader = this.$loading.show();
    try {
      await this.RequestInstance.fetchReceiveRequest([this.perPage, pageIndex]);
      this.requests = this.RequestInstance.getReceiveRequest.requests;
      if (this.requests !== null) {
        this.pageCount = Math.ceil(
          (this.RequestInstance.getReceiveRequest.totalRecord || this.perPage) /
            this.perPage
        );
      }
    } catch (e) {
